...
func Check() {
	DrawGuessedWord.GuessedWordPixels = InitializeGuessedWord(len(DrawGuessedWord.Randomword), 25, 17)
	DrawGuessedWord.GuessedWordPixels = RevealRandomLetters(DrawGuessedWord.GuessedWordPixels, DrawGuessedWord.Randomword)
	updatesecondwindows.Update()
	reader := bufio.NewReader(os.Stdin)
	attemptsLeft := 10
	var wordUse string

	for !IsWordGuessed(DrawGuessedWord.GuessedWordPixels, DrawGuessedWord.Randomword) && attemptsLeft > 0 {
		SetCursorPos(40, 12)
		...
		if strings.Contains(strings.ToLower(DrawGuessedWord.Randomword), strings.ToLower(guess)) {
			DrawGuessedWord.GuessedWordPixels = RevealLetter(guess, DrawGuessedWord.Randomword, DrawGuessedWord.GuessedWordPixels)
			updatesecondwindows.Update()
	...
	if !IsWordGuessed(DrawGuessedWord.GuessedWordPixels, DrawGuessedWord.Randomword) && attemptsLeft == 0 {
		// Révélez le mot entier
		for i := range DrawGuessedWord.GuessedWordPixels {
			DrawGuessedWord.GuessedWordPixels[i].Color = string(DrawGuessedWord.Randomword[i]) + " "
		}
		updatesecondwindows.UpdateDisplay(attemptsLeft)
		fmt.Print(" \r")
		DrawPhraseLoose(DrawGuessedWord.Randomword)
		time.Sleep(15 * time.Second)
		Functions.Clear()
	} else if IsWordGuessed(DrawGuessedWord.GuessedWordPixels, DrawGuessedWord.Randomword) {
		fmt.Print(" \r")
		DrawPhraseWin()
		time.Sleep(15 * time.Second)
		Functions.Clear()
